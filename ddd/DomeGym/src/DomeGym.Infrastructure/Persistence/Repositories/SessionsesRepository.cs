...
public class SessionsesRepository : ISessionsRepository
{
    private readonly DomeGymDbContext _dbContext;

    public SessionsesRepository(DomeGymDbContext dbContext)
    {
    ...
    public async Task<List<Session>> ListByIdsAsync(IReadOnlyList<Guid> sessionIds, DateTime? startDateTime = null,
        ...
        List<SessionCategory>? categories = null)
    {
        return await _dbContext.Sessions.AsNoTracking()
            ...
            .WhereOfCategories(categories)
            .WhereOfCategories(categories)
            .ToListAsync();
    }

    public async Task<List<Session>> ListByGymIdAsync(Guid gymId, DateTime? startDateTime = null,
        DateTime? endDateTime = null,
        List<SessionCategory>? categories = null)
    {
        var gymRooms = await _dbContext.Rooms.AsNoTracking()
            .Where(room => room.GymId == gymId)
            .ToListAsync();
        var sessionIds = gymRooms.SelectMany(room => room.SessionIds)
            .ToList();
    ...
    {
        return await _dbContext.Sessions.Where(session => session.RoomId == roomId)
            .ToListAsync();
    }
...
{
    public static IQueryable<Session> WhereBetweenDateAndTimes(
    ...
    public static IQueryable<Session> WhereOfCategories(this IQueryable<Session> query,
