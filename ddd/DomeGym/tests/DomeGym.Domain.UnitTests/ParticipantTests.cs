...
public class ParticipantTests
{
    ...
    )
    {
        ...
        // Act
        var addSession1Result = participant.AddToSchedule(session: session1);
        var addSession2Result = participant.AddToSchedule(session: session2);

        // Assert
        addSession1Result.IsError.Should()
            .BeFalse();
        addSession2Result.FirstError.Should()
            .Be(ParticipantErrors.CannotHaveTwoOrMoreOverlappingSessions);
    }
}